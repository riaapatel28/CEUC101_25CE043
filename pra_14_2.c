...
void main()
{
	float balance=5000.0,wamount,money,y;
	int choice,pin,entered_pin,withdrawcount=0;
	pin=1234;
	printf("enter the pin number=");
	scanf("%d",&entered_pin);
	if(entered_pin!=pin)
    {
        printf("entered pin number is wrong!you can't access");
	}
	    ...
	    if(choice==2)
    {
        if(withdrawcount>=3)
        {
            printf("daily withdraw limit reached\n");
            continue;
        }
        withdrawcount++;
    }
